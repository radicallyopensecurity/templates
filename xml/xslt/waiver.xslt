...
<fo:block xsl:use-attribute-sets="p">A. <xsl:value-of select="../client/short_name"/> wants some of its systems to be tested, <xsl:value-of select="/offerte/meta/company/full_name"/> (“<xsl:value-of select="/offerte/meta/company/short_name"/>”) has offered to perform such testing for <xsl:value-of select="./client/short_name"/> and <xsl:value-of select="./client/short_name"/> has accepted this offer. The assignment will be performed by <xsl:value-of select="/offerte/meta/company/short_name"/>' core-team members, external freelancers, and/or volunteers (the “Consultants”).</fo:block>
<fo:block xsl:use-attribute-sets="p">B. Some of the activities performed by <xsl:value-of select="/offerte/meta/company/short_name"/> and the Consultants during the course of this assignment could be considered illegal, unless <xsl:value-of select="short_name"/> has given permission for these activities. <xsl:value-of select="/offerte/meta/company/short_name"/> and the Consultant will only perform such activities if they have received the required permission.</fo:block>
...
<fo:block xsl:use-attribute-sets="p">5. Should the public prosecutor initiate an investigation or criminal proceedings against <xsl:value-of select="/offerte/meta/company/short_name"/> or any of the consultants it engaged or employed as a result of the performance of the assignment for the customer, then <xsl:value-of select="short_name"/> will co-operate fully with <xsl:value-of select="/offerte/meta/company/short_name"/> in defending against this investigation or proceedings, including by providing any evidence it has which relates to this investigation or these proceedings.</fo:block>
